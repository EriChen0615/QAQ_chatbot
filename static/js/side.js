...
function send(text) {
	...
	$.ajax({
		...
		newReceivedMessage(data["msg"]);
		var node = document.getElementById('faqBox');
        node.innerHTML = "";
		data["faq"].forEach(createNewFAQ);

		},
		error: function() {
			newReceivedMessage("Internal Server Error");
		}
	});
}
...
function newReceivedMessage(messageText) {
	...
	lastReceivedMessage = removedQuotes;
...
function createNewMessage(message) {
	...
	$chatlogs.append(
		$('<div/>', {'class': 'chat friend'}).append(
			$('<div/>', {'class': 'user-photo'}).append($('<img src="static/images/bot.png" />')),
